    ...
    let ctx = gsap.context(() => {
      ...
      languages.forEach((_, index) => {
        tl.to(textRef.current, {
          y: `-${index * 100}%`, // Move to the next language
          duration: 2, // Animation duration for each language
          ease: "linear",
        }).addPause("+=1"); // Pause for 1 second after each move
      });
  ...
  return (
    <div>
      ...
      <div className="hws-container">
            <p className="hws-text">
              System.out.println('<span className="scrolling-text-container">
                <span ref={textRef} className="scrolling-text">
                  {languages.map((text, index) => (
                    <span key={index} className="scrolling-item">{text}</span>
                  ))}
                </span>
              </span>')
            </p>
